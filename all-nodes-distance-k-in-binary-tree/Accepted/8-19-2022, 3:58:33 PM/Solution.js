...
const distanceK = function(root, target, K) {
    ...
    const getParent = (root, map, visited) => {
        if (!root) return
        ...
        if (root.left) {
            ...
            getParent(root.left, map, visited)
        }
        if (root.right) {
            map.set(root.right, root)
            getParent(root.right, map, visited)
        }
    }
    
    const findNodes = (root, distance = 0) => {
